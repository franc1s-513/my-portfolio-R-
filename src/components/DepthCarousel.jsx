...
const DepthCarousel = ({
  ...
  const onCardClick = useCallback(
    index => {
      ...
      // If it's already focused, fire onItemClick so the parent can open a modal
      if (focusRef.current === index && onItemClick) {
        ...
        {data.map((item, i) => (
          <div
            ...
            style={{ 
              ...
              height: cardHeight, 
              borderRadius: radius,
              border: item.color ? `2px solid ${item.color}44` : 'none',
              boxShadow: item.color ? `0 0 20px ${item.color}22` : ''
            }}
          ...
          >
            ...
            {item.title && (
              <div className="depth-carousel__cert-header">
                <p className="depth-carousel__cert-title" style={{color: item.color}}>{item.title}</p>
                <p className="depth-carousel__cert-issuer">{item.issuer}</p>
              </div>
            )}
